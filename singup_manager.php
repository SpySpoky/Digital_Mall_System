...
if (isset($_POST['SubmitSignUpManager'])) {
    ...
    if($password === $confirm_pass) {
        ...
        if(mysqli_num_rows($result_email) > 0) {
            ...
            if($result) {
                ...
                $sql_new_manager = "SELECT * from users1 where email = '$email'";
                $result_new_manager = mysqli_query($conn, $sql_new_manager);
                $fetched_user = mysqli_fetch_assoc($result_new_manager);

                $_SESSION['temp_user_id'] = $fetched_user['id'];
                $_SESSION['temp_user_email'] = $fetched_user['email'];
...
<head>
    ...
    <title>Digital Mall - Sign-up for Managers</title>
...
<body class="bg-gradient-to-br from-yellow-100 via-yellow-200 to-purple-200 flex items-center justify-center min-h-screen p-6">
    ...
    <div class="bg-white p-8 rounded-2xl shadow-lg w-full max-w-md">
        <div class="text-center mb-6">
            ...
            <p class="text-gray-500 text-sm">Create your manager account</p>
        </div>
        ...
        <form action="" method="POST" id="signupForm" class="space-y-4">
            ...
            <button id="signupBtn" type="submit" name="SubmitSignUpManager"class="w-full bg-purple-700 text-white p-3 rounded-lg font-semibold hover:bg-purple-800 hover:scale-[1.02] active:scale-[0.98] transition">
                Sign Up
            </button>
    ...
    <script src="js/singup_manager.js"></script>
</body>
</html>